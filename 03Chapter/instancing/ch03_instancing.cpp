...
//声明InstancingExample类-----具体代码在vapp.h文件中
BEGIN_APP_DECLARATION(InstancingExample)
    ...
    VBObject object;
END_APP_DECLARATION()
//end 声明InstancingExample类

//用DEFINE_APP宏定义main函数以及基类DisplayFunc函数与ReshapeFunc函数，MainLoop函数
//具体内容详见vapp.h文件
...
void InstancingExample::Initialize(const char * title)
{
	...
	//如glVertexAttribDivisor定义，每一个实例有一个相应的权重值
	//注意：上面的glBufferData函数的最后一个参数为GL_DYNAMIC_DRAW，动态写入
